...
function computeSuggestedGuardians(theme: string, claudeSuggestions: string[]): string[] {
  // Step 1: テーマに直接名前が含まれる守護存在を最優先
  const mentioned: string[] = []
  for (const [name, id] of Object.entries(GUARDIAN_NAME_TO_ID)) {
    if (theme.includes(name) && !mentioned.includes(id)) {
      mentioned.push(id)
    }
  }

  // Step 2: GUARDIANSのキーワードとテーマの一致スコアで順位付け
  const scored = GUARDIANS
    .filter(g => !mentioned.includes(g.id))
    .map(g => ({
      id: g.id,
  ...
  // Step 3: 名前一致 → キーワード一致 → AIの提案 の優先順でユニーク3体に絞る
  const seen = new Set<string>()
  const result: string[] = []
  for (const id of [...mentioned, ...keywordMatches, ...claudeSuggestions]) {
    if (!seen.has(id)) {
      seen.add(id)
      result.push(id)
    }
...
export async function POST(req: NextRequest) {
  try {
    const { theme, postType, targetSNS } = await req.json()
