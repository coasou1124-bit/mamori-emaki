...
function findMentionedGuardian(text: string): string | null {
...
使えるIDのみ: ${guardianIds}`
    ...
    // テーマ文に守護存在名が含まれていれば最優先で先頭へ
    const mentionedId = findMentionedGuardian(theme)
    if (mentionedId && Array.isArray(result.suggestedGuardians)) {
      const rest = (result.suggestedGuardians as string[]).filter((id) => id !== mentionedId)
      result.suggestedGuardians = [mentionedId, ...rest].slice(0, 3)
    }
